...
function symbolic_interpolation_matrix!(mat, dest, src, baryweights, simplify_fn)
    ...
    @inbounds for k in axes(mat, 1)
        row_has_match = false
        for j in axes(mat, 2)
            # Exact equality needed — no rtoldefault for symbolic types.
            if dest[k] == src[j]
                row_has_match = true
                mat[k, j] = 1
            end
    ...
    @inbounds for idx in eachindex(mat)
        mat[idx] = simplify_fn(mat[idx])
    end
    nothing
end

function symbolic_lobatto_legendre(p, T, simplify_fn)
    if p == 0
        nodes   = T[0]
        weights = T[2]
    elseif p == 1
    ...
    baryweights_sym = simplify_fn.(barycentric_weights(nodes))
    D = simplify_fn.(derivative_matrix(nodes, baryweights_sym))
    LobattoLegendre(nodes, weights, baryweights_sym, D)
end

function symbolic_gauss_legendre(p, T, simplify_fn)
    if p == 0
        nodes   = T[0]
        weights = T[2]
    elseif p == 1
...
function symbolic_gauss_radau_left(p, T, simplify_fn)
    if p == 0
        nodes   = T[-1]
        weights = T[2]
    elseif p == 1
...
function symbolic_gauss_radau_right(p, T, simplify_fn)
    if p == 0
        nodes   = T[1]
        weights = T[2]
    elseif p == 1
