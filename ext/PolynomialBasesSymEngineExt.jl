...
function PolynomialBases.LobattoLegendre(p::Int, T::Type{SymEngine.Basic})
    if p == 0
        nodes   = T[0]
        weights = T[2]
    elseif p == 1
    ...
    baryweights_sym = SymEngine.expand.(barycentric_weights(nodes))
    D = SymEngine.expand.(derivative_matrix(nodes, baryweights_sym))
    LobattoLegendre(nodes, weights, baryweights_sym, D)
end

function PolynomialBases.GaussLegendre(p::Int, T::Type{SymEngine.Basic})
    if p == 0
        nodes   = T[0]
        weights = T[2]
    elseif p == 1
...
function PolynomialBases.GaussRadauLeft(p::Int, T::Type{SymEngine.Basic})
    if p == 0
        nodes   = T[-1]
        weights = T[2]
    elseif p == 1
...
function PolynomialBases.GaussRadauRight(p::Int, T::Type{SymEngine.Basic})
    if p == 0
        nodes   = T[1]
        weights = T[2]
    elseif p == 1
