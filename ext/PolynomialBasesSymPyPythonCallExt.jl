...
function PolynomialBases.interpolation_matrix!(mat, dest, src::AbstractVector{<:SymPyPythonCall.Sym}, baryweights)
    ...
    @inbounds for k in axes(mat, 1)
        row_has_match = false
        for j in axes(mat, 2)
            # no method matching rtoldefault(::Type{<:SymPyPythonCall.Sym})
            if dest[k] == src[j]
                row_has_match = true
                mat[k, j] = 1
            end
    ...
    @inbounds for idx in eachindex(mat)
        mat[idx] = SymPyPythonCall.simplify(mat[idx])
    end
    nothing
end

function PolynomialBases.LobattoLegendre(p::Int, ::Type{<:SymPyPythonCall.Sym})
    T = _Sym
    if p == 0
        nodes   = T[0]
        weights = T[2]
    ...
    baryweights_sym = SymPyPythonCall.simplify.(barycentric_weights(nodes))
    D = SymPyPythonCall.simplify.(derivative_matrix(nodes, baryweights_sym))
    LobattoLegendre(nodes, weights, baryweights_sym, D)
end

function PolynomialBases.GaussLegendre(p::Int, ::Type{<:SymPyPythonCall.Sym})
    T = _Sym
    if p == 0
        nodes   = T[0]
        weights = T[2]
...
function PolynomialBases.GaussRadauLeft(p::Int, ::Type{<:SymPyPythonCall.Sym})
    T = _Sym
    if p == 0
        nodes   = T[-1]
        weights = T[2]
...
function PolynomialBases.GaussRadauRight(p::Int, ::Type{<:SymPyPythonCall.Sym})
    T = _Sym
    if p == 0
        nodes   = T[1]
        weights = T[2]
